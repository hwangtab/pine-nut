...
export const FEAST_LINEUP: ConcertArtist[] = [
  ...
  },
  {
    ...
    // 윤석열 퇴진 집회는 넣기로 했다 — 이 사람이 실제로 선 자리이고, 이력서에
    // 본인이 적어둔 것이다. 이력서에는 팔레스타인 연대 집회 공연도 있는데,
    // 네 자리를 넘기면 목록이 되어 읽히지 않아 넣지 않았다.
    blurb:
      "독학으로 음악을 익힌 싱어송라이터. 아버지 성 ‘최’와 어머니 성 ‘양’에 ‘다음’을 붙인 이름으로, 호주제에 맞선다는 뜻을 담아 지었습니다. 그 이름을 여러 나라 말로 씁니다 — 다음, NEXT, 次, Nächste, 翌. 세월호 10주기 추모, 수요시위, 윤석열 퇴진 집회, 5·18 기념식 같은 자리에서 노래해왔습니다.",
