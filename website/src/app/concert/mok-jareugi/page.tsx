...
// 구조화 데이터. 포스터를 놓친 사람이 검색으로 찾아오는 경로다. 종료 시각은 아직
// 정해지지 않았으므로 endDate 를 넣지 않는다 — 모르는 값을 지어내면 리치결과에 틀린
// 시간이 박힌다.
const eventJsonLd = {
...
const BRIEFING = [
  ...
  {
    q: "무엇이 사라지나",
    a: "산림청이 ‘100대 명품숲’으로 지정한 1,800ha 잣나무 숲이 훼손됩니다. 벌채 예정 잣나무는 약 11만 그루, 51가구가 살던 터가 물에 잠깁니다. 주민들은 이 숲에서 산양을 봤다고 증언합니다.",
  },
  {
    q: "왜 잣나무인가",
    a: "풍천리는 잣 생산지입니다. 주민들에게 이 숲은 경치가 아니라 생계입니다. 나무가 없어지면 수입이 없어지고, 마을에 남을 이유도 없어집니다.",
  },
  {
    q: "누가 반대하나",
...
const PARTICIPATE = [
  ...
  },
  {
    icon: Users,
    title: "현장에서, 함께해요",
    body: "자리를 채우는 것 자체가 연대입니다. 베어질 숲을 직접 보고, 사진을 찍어 ‘#풍천리’로 남겨주세요.",
  },
  {
    icon: Heart,
    title: "못 오셔도, 연대해요",
...
const FAQ = [
  ...
  {
    q: "몇 시에 시작하고 언제 끝나나요?",
    a: "오후 2시에 시작합니다. 팀별 순서와 종료 시각은 아직 정해지지 않았습니다. 정해지는 대로 이 페이지에 올리겠습니다.",
  },
  {
    q: "풍천리가 어디인가요?",
          ...
          <dl className="mt-14 grid gap-x-10 gap-y-8 sm:grid-cols-2">
            {BRIEFING.map((item) => (
              <div key={item.q}>
                ...
                <dd className="mt-2 break-keep text-[15px] leading-relaxed text-white/75">
                  {item.a}
                </dd>
              </div>
            ))}
      ...
      <section id="lineup" className="scroll-mt-20 bg-[var(--color-bg-moss)] px-6 py-16 sm:py-24">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="mt-3 break-keep text-sm leading-relaxed text-[var(--color-text-muted)]">
            포스터에 적힌 순서입니다. 오후 2시에 시작하지만 팀별 시각은 아직 정해지지
            않았습니다. 정해지는 대로 이 자리에 올리겠습니다.
          </p>
