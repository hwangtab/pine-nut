...
export default function NicknameForm({ current }: { current: string | null }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 sm:flex-row sm:items-center">
      <input name="nickname" defaultValue={current ?? ""} placeholder="닉네임(2~20자)" className="px-4 py-2 text-base border border-[var(--color-admin-border)] rounded-xl outline-none focus:ring-2 focus:ring-[var(--color-forest)]/30" />
      <SaveButton />
